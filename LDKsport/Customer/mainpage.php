...
<body>
    ...
    <?php
    $latest_products = mysqli_query($conn, "SELECT * FROM product ORDER BY created_at DESC LIMIT 4");
    if (mysqli_num_rows($latest_products) > 0) {
        echo '<section class="latest-products">';
        echo '<div class="products-banner">';
        while ($product = mysqli_fetch_assoc($latest_products)) {
            ?>
